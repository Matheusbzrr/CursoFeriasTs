...
export default class GeneroController {
    ...
    async create(req: Request, res: Response) {
        ...
        try {
            ...
            const savedGenero = await generoRepository.criar(genero);
    ...
    async findAll(req: Request, res: Response) {
        try {
            const generos = await generoRepository.buscarAll();
    ...
    async findOne(req: Request, res: Response) {
        ...
        try {
            const genero = await generoRepository.buscarById(id);
    ...
    async findName(req: Request, res: Response) {
        ...
        try {
            const genero = await generoRepository.buscarByNome(nome);
            if (genero) res.status(200).send(genero);
            else
                res.status(404).send({
            ...
            res.status(500).send({
                message: `Error não foi possível retornar o gênero com esse nome=${nome}`
            });
        }
    ...
    async delete(req: Request, res: Response) {
        ...
        try {
            ...
            if (num == 1) {
                ...
                res.send({
                    message:`Não foi possível deletar o Gênero com id=${id}. O Gênero não foi encontrado.,`
                });
            }
        } catch (err) {
            res.status(500).send({
